...
: Host-side, secretless software that routes signing requests and
  ...
  app, local service, NIP-46 bridge, and NIP-46 relay session.
...
: Companion-side conversion of already-decrypted NIP-46 payloads into
  standard nSealr signing requests. The bridge itself does not run relay
  sessions or encryption — those are the relay session layer
  (`nsealr-nip46-session-active-v0`).

**NIP-46 relay session** — `nsealr-nip46-session-active-v0`
: Companion-side NIP-46 remote-signer session over Nostr relays: relay
  I/O, NIP-44 v2 encryption, request/response correlation, and BIP-340
  verification, using an ephemeral client transport key. The user signing
